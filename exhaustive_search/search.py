#!/usr/bin/env python3
"""Run exhaustive search for Cayley (n, k, t, lambda, mu)-DSRGs over a single parameter set. Searches every possible subset of every nonabelian group of order n."""
...
def run_single(
    ...
    for gi, group in enumerate(groups):
        ...
        if found_subsets:
            ...
            npz_file = (
                task_dir / f"dsrg_{n}_{k}_{t}_{lambda_}_{mu}_g{group.library_id}.npz"
            )
...
def build_cli_parser() -> ArgumentParser:
    ...
    _ = parser.add_argument(
        "positional", nargs="*", type=int, metavar="N", help="n k t lambda mu"
    )
    _ = parser.add_argument(
        "--params",
        type=Path,
        default=None,
        help="CSV with n, k, t, lambda, mu columns",
    )
    _ = parser.add_argument(
    ...
    _ = parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("cayley_data"),
        help="Base output directory",
    )
    _ = parser.add_argument(
        "--batch-size", type=int, default=100_000, help="Subsets per GPU batch"
    )
    _ = parser.add_argument(
        "--noninteractive",
        action="store_true",
...
# TODO handle mutual exclusion in arguments correctly
...
if __name__ == "__main__":
    ...
    if args.params is not None:
        ...
        if args.index >= len(params_df):
            print(
                f"Error: index {args.index} out of range (file has {len(params_df)} rows)"
            )
            sys.exit(1)
