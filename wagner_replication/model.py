...
class WagnerModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, i: torch.Tensor) -> torch.Tensor:
        assert i.dim() == 0 or (i.dim() == 1 and i.numel() == 1), "i must be a singleton tensor"
        assert i.item() < self.edges, f"i ({i.item()}) must be less than number of edges ({self.edges})"
...
def select_elite(constructions: torch.Tensor, batch_scores: torch.Tensor, elite_proportion: float=.1):
    ...
    return torch.tensor([constructions[i] for i in elite_indices]).to(constructions.device)

def extract_examples(elite_constructions: torch.Tensor, output_batch_size=64) -> DataLoader:
