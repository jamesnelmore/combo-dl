...
# Run RRHCkill over SmallGroup(n, i) for i in [startIdx..endIdx] and append
# one result line per group to outFile. Used by the SLURM array driver.
# Output line format (space-separated):
#   n lib_id k t lambda mu STATUS [difference_set]
# where STATUS is FOUND or NONE.

RRHCindices:= function(n, startIdx, endIdx, k, t, lambda, mu, kill, outFile)

local i, G, l;

for i in [startIdx..endIdx] do
	G:= SmallGroup(n, i);
	Print("***********************", "\n", "n=", n, " id=", i, " k=", k, " t=", t, " lambda=", lambda, " mu=", mu, "\n", "***********************", "\n");
	l:= RRHCkill(G, k, t, lambda, mu, kill);
	if not IsEmpty(l) then
		AppendTo(outFile, n, " ", i, " ", k, " ", t, " ", lambda, " ", mu, " FOUND ", l, "\n");
	else
		AppendTo(outFile, n, " ", i, " ", k, " ", t, " ", lambda, " ", mu, " NONE\n");
	fi;
od;
