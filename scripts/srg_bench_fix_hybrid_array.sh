#!/bin/bash
#SBATCH --array=0-35
#SBATCH --job-name=srg-fix-hybrid
...
# SRG ILP Benchmark: exact formulation with fix-neighbors + hybrid lex
# (block_size=10: exponential within blocks, lex-leader between blocks).
#
# Usage:
#   sbatch scripts/srg_bench_fix_hybrid_array.sh
...
# ── Logging ───────────────────────────────────────────────────────────────
echo "=== SRG ILP Benchmark (fix-neighbors + hybrid lex) ==="
...
python -m ilp bench-single \
    ...
    --model "${MODEL}" \
    --fix-neighbors --lex hybrid --lex-block-size 10 \
